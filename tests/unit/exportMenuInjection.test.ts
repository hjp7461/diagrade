...
describe('injectExportMenus', () => {
  ...
  it('각 정상 mermaid 컨테이너에 메뉴를 주입한다', () => {
    ...
    injectExportMenus(container, { activeTabPath: '/x/note.md' });
  ...
  it('FR-28: .diagrade-mermaid-error 에는 주입하지 않는다', () => {
    ...
    injectExportMenus(container, { activeTabPath: null });
  ...
  it('mermaid 컨테이너가 0 개면 no-op', () => {
    ...
    expect(() => injectExportMenus(container, { activeTabPath: null })).not.toThrow();
